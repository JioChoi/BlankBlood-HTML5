...
if(step==4){
	...
	if(!dir) col2 = collision_rectangle(x+15,y-20,x-85,y+30,objBreakBlock,0,1);
	else col2 = collision_rectangle(x-15,y-20,x+85,y+30,objBreakBlock,0,1);
	while (col2 != noone) {
		with(col2){
			HP -= 30+random(10);
			if(HP<=0) alarm[11] = 1;
...
if(step == 3){
	...
	if(!dir){
		...
		cnt = 0;
		// �^�[�Q�b�g�ƂȂ�I�u�W�F�N�g�̃C���X�^���X��T��
		col = collision_rectangle(x+15,y-20,x-85,y+30,objEnemy,0,1);
		while (col != noone) {
			with(col){
				hspd = -200/weight;
				vspeed = -100/weight;
